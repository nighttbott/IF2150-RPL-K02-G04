...
| C02      | `MateriAksara`         | (Entity) Menyimpan data modul, jenis aksara, dan tingkat kesulitan materi                       				 |
| C03      | `RiwayatLatihan` 	    | (Entity) Menyimpan rekam hasil pengerjaan latihan dan status progres pelajar                                   	         |
| C07      | `HalamanUtama`         | (Boundary) Papan navigasi awal setelah log in yang menjadi pintu masuk ke fitur-fitur lain                                 |
| C14      | `HalamanProgres`       | (Boundary) Antarmuka riwayat, penguasaan materi, rekomendasi, dan indikator motivasi bagi pelajar                          |
| C19      | `ProgresController`    | (Controller) Menampilkan riwayat latihan, mengirim rekomendasi materi, dan menyiapkan data papan peringkat	         |
| C22      | `Motivasi Belajar`     | (Entity) Menyimpan streak harian, poin, dan badge yang dimiliki seorang pelajar                                            |
...
| ID Kelas | Nama Kelas | Atribut | Metode/Operasi |
| :------- | :--------------------- | :----------------------------------- | :--------------------------------------- |
| C01 | `AkunPengguna` | _idAkun, namaTampilan, peran_ | _-_ |
| C02 | `MateriAksara` | _idMateri, namaAksara, jenisAksara, tingkatKesulitan_ | _-_ |
| C03 | `RiwayatLatihan` | _idRiwayat, mode, skor, nilaiAkurasi, statusProgres, tanggalPengerjaan_ | _hitungTingkatPenguasaan()_ |
| C07 | `HalamanUtama` | _-_ | _bukaMenuProgres()_ |
| C14 | `HalamanProgres` | _-_ | _bukaHalamanProgres(), pilihMateriRiwayat(idMateri), mintaRekomendasi(), tampilkanIndikatorMotivasi(), bukaPapanPeringkat()_ |
| C19 | `ProgresController` | _-_ | _tampilkanRiwayat(idPelajar), rekomendasikanMateri(idPelajar), tampilkanMotivasi(idPelajar), siapkanPapanPeringkat(idKelas)_ |
| C22 | `MotivasiBelajar` | _idMotivasi, streakHarian, totalPoin, daftarBadge_ | _ringkasanMotivasi()_ |
...
`HalamanKelolaMateri` bergantung (dependensi) pada `MateriController` karena antarmuka pengelolaan materi memanggil logika penambahan, validasi format, dan pembaruan materi tanpa menyimpan data secara langsung. `MateriController` berasosiasi dengan `MateriAksara` untuk mengambil, memvalidasi, dan memperbarui data materi aksara di _database_ (sesuai KF15 dan KF16).
...
| ID Kelas | Nama Kelas | Deskripsi Kelas |
| :--- | :--- | :--- |
| C26 | `HalamanKelolaKelas` | *(Boundary)* Antarmuka bagi pengajar untuk mengelola kelas, kode, anggota, dan tugas. |
| C28 | `KelasController` | *(Controller)* Mengoordinasikan pembuatan kelas, pengelolaan kode/anggota, dan penerbitan tugas. |
| C20 | `KelasBelajar` | *(Entity)* Menyimpan data kelas, kode aktif, dan pengaturan papan peringkat milik pengajar. |
| C21 | `KeanggotaanKelas` | *(Entity)* Menyimpan status keanggotaan pelajar pada kelas yang dikelola pengajar. |
| C22 | `TugasKelas` | *(Entity)* Menyimpan tugas kelas beserta tenggat dan komponen yang ditetapkan pengajar. |
...
`HalamanKelolaKelas` bergantung (dependency) pada `KelasController` untuk memproses pembuatan dan pengelolaan kelas oleh pengajar. `KelasController` berasosiasi dengan `KelasBelajar` untuk membuat kelas dan mengelola kode aktif (KF32) serta pengaturan papan peringkat (KF38). `KelasBelajar` berkomposisi dengan `KeanggotaanKelas` (mengelola anggota, termasuk mengeluarkan anggota) dan dengan `TugasKelas` (menerbitkan tugas beserta tenggat, KF34), karena keduanya melekat pada satu kelas tertentu dan tidak bermakna tanpa kelas induknya.

| ID Kelas | Nama Kelas | Atribut | Metode/Operasi |
| :--- | :--- | :--- | :--- |
| C26 | `HalamanKelolaKelas` | *-* | *buatKelas(), rotasiKode(), keluarkanAnggota(), terbitkanTugas()* |
| C28 | `KelasController` | *-* | *kelolaKelas(perintah), terbitkanTugas(data)* |
| C20 | `KelasBelajar` | *idKelas, nama, kodeAktif, papanPeringkatAktif, pemilikId* | *rotasiKode(), aturPapanPeringkat()* |
| C21 | `KeanggotaanKelas` | *idKeanggotaan, bergabungPada, status* | *akhiri()* |
| C22 | `TugasKelas` | *idTugas, judul, tenggat, daftarKomponen* | *terbitkan()* |
...
<p align="center">
<img alt="Class Diagram Keseluruhan" src="./assets/diagram/classDiagram.png" width="70%">
</p>
<p align="center">
